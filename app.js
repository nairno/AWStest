...
const options = {
  ...
  client.on('chat', (channel,user, message, self, badges) => {
    ...
    if (message === '!game'){
      client.action('nairno','chatfightbot is playing a game');
      if (self) return;
      console.log(user)
    }
  ...
  //try to fire the quiz
    ...
    if (canQuizPop == false){
      client.action('nairno','No quiz yet');
      console.log(func)
    }
    else if (canQuizPop == true){
      client.action('nairno','Lets quiz');
    }
  ...
  if (message == '!battle'){
    ...
    if(battleFire == true){
      ...
      console.log(randomNumber)
      console.log(`${user['id']}`)
      if ('nairno', `${user['display-name']}` != championVal){
    //if random number is above 50 then they win the battle
        if (randomNumber >= 50){
          client.say('nairno', `${user['display-name']} you are the new champion`);
          console.log (randomNumber)
          ...
          const battleFuncWin = () => {
            ...
            client.say('nairno', 'You may now challenge the champion in a fight to the death');
          };
    ...
    //put in switches for the losing fights
          ...
          client.say('nairno', `${user['display-name']} you lost the battle...just`);
          console.log (randomNumber)
          battleFire = false
        ...
        client.say('nairno', `${user['display-name']} you lost the battle...it was a tough fight`);
        console.log (randomNumber)
        battleFire = false
        const battleFuncLose = () => {
          console.log('lose timer finish');
      ...
      client.say('nairno', `${user['display-name']} you lost the battle...it may have looked close, but it wasn't`);
      console.log (randomNumber)
      battleFire = false
      const battleFuncLose = () => {
        console.log('lose timer finish');
      ...
      setTimeout(battleFuncLose, 4 * 10000);
        ...
        client.say('nairno', `${user['display-name']} you lost the battle...really badly`);
        console.log (randomNumber)
        battleFire = false
        const battleFuncLose = () => {
    ...
    setTimeout(battleFuncLose, 4 * 10000);
      ...
      client.say('nairno', `${user['display-name']} you lost the battle...you are the worst`);
      console.log (randomNumber)
      battleFire = false
      const battleFuncLose = () => {
        console.log('lose timser finish');
  ...
  }else if ('nairno', `${user['display-name']}` == championVal){
    championVal = champion[champion.length - 1];
    client.say('nairno',`${user['display-name']}` + ' you are the current champion');
  ...
  client.say('nairno', 'you must wait for the next battle');
    }
  }
  ...
  if (message == '!champion'){
    ...
    championVal = champion[champion.length - 1];
    console.log(championVal)
    client.say('nairno', championVal + ' is the current champion');
  }
